...
void timer_init_detailed(uint32_t prescaler, uint32_t bitmode,
  ...
  register_write((TIMER0_BASE_ADDRESS + TIMER_SHORTS),
                 0x01); //  A=1 -- CLEAR on CC[0]

  // Start the Module -------------------------------------------------------

  // Enable Interrupt
  register_write((TIMER0_BASE_ADDRESS + TIMER_INTENSET),
                 INT_COMPARE0); // Interrupt on Compare[0]
